...
const HODModule = (() => {
    ...
    function renderFaculty(c) {
        ...
        /* Edit faculty */
        c.querySelectorAll('.edit-faculty').forEach(btn => {
            btn.addEventListener('click', () => {
                const fac = DataStore.getFacultyById(btn.dataset.id);
                if (fac) showEditFacultyModal(c, depts, fac);
            });
        });
    ...
    function showEditFacultyModal(c, depts, fac) {
        const area = document.getElementById('faculty-modal-area');
        area.innerHTML = `
        <div class="inline-modal">
            <div class="modal-overlay" id="ef-overlay"></div>
            <div class="modal-content">
                <div class="modal-header">
                    <h2>Edit Faculty</h2>
                    <button class="modal-close" id="ef-close">&times;</button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label class="form-label">Full Name</label>
                        <input type="text" class="form-input" id="ef-name" value="${fac.name}">
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label class="form-label">Username</label>
                            <input type="text" class="form-input" id="ef-username" value="${fac.username}">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Password</label>
                            <input type="text" class="form-input" id="ef-password" value="${fac.password || ''}" placeholder="Password">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Email</label>
                        <input type="email" class="form-input" id="ef-email" value="${fac.email || ''}">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Department</label>
                        <select class="form-select" id="ef-dept">
                            <option value="">— Select Department —</option>
                            ${depts.map(d => `<option value="${d.id}" ${d.id === fac.departmentId ? 'selected' : ''}>${d.name}</option>`).join('')}
                        </select>
                    </div>
                    <div id="ef-error" class="error-message" style="display:none"></div>
                </div>
                <div class="modal-footer">
        ...
        const closeFn = () => { area.innerHTML = ''; };
        document.getElementById('ef-close').addEventListener('click', closeFn);
        document.getElementById('ef-overlay').addEventListener('click', closeFn);
        document.getElementById('ef-cancel').addEventListener('click', closeFn);
        ...
        document.getElementById('ef-save').addEventListener('click', () => {
            ...
            const password = document.getElementById('ef-password').value.trim();
            const email    = document.getElementById('ef-email').value.trim();
            const deptId   = document.getElementById('ef-dept').value;
            const errEl    = document.getElementById('ef-error');

            if (!name || !username || !deptId) {
                errEl.textContent = 'Please fill in all required fields.';
                errEl.style.display = 'block';
                return;
            }

            const existing = DataStore.getFaculty().find(f => f.username === username && f.id !== fac.id);
            if (existing) {
                errEl.textContent = 'Username already exists. Choose a different one.';
                errEl.style.display = 'block';
                return;
            ...
            DataStore.updateFaculty(fac.id, { name, username, password, email, departmentId: deptId });
            closeFn();
    ...
    function renderDepartments(c) {
        ...
        /* Edit */
        c.querySelectorAll('.edit-dept').forEach(btn => {
            btn.addEventListener('click', () => {
                const newName = prompt('Edit department name:', btn.dataset.name);
                if (newName && newName.trim() && newName.trim() !== btn.dataset.name) {
                    DataStore.updateDepartment(btn.dataset.id, { name: newName.trim() });
    ...
    function renderSubjects(c) {
        ...
        /* Edit */
        c.querySelectorAll('.edit-subject').forEach(btn => {
            btn.addEventListener('click', () => {
                const sub = DataStore.getSubjectById(btn.dataset.id);
                if (sub) showEditSubjectModal(c, depts, sub);
            });
        });
    ...
    function showEditSubjectModal(c, depts, sub) {
        const area = document.getElementById('subject-modal-area');
        area.innerHTML = `
        <div class="inline-modal">
            <div class="modal-overlay" id="es-overlay"></div>
            <div class="modal-content">
                <div class="modal-header">
                    <h2>Edit Subject</h2>
                    <button class="modal-close" id="es-close">&times;</button>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label class="form-label">Subject Name</label>
                        <input type="text" class="form-input" id="es-name" value="${sub.name}">
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label class="form-label">Subject Code</label>
                            <input type="text" class="form-input" id="es-code" value="${sub.code}">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Credits</label>
                            <input type="number" class="form-input" id="es-credits" value="${sub.credits}" min="1" max="6">
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label class="form-label">Department</label>
                            <select class="form-select" id="es-dept">
                                <option value="">— Select —</option>
                                ${depts.map(d => `<option value="${d.id}" ${d.id === sub.departmentId ? 'selected' : ''}>${d.name}</option>`).join('')}
                            </select>
                        </div>
                        <div class="form-group">
                            <label class="form-label">Semester</label>
                            <select class="form-select" id="es-semester">
                                ${[1,2,3,4,5,6,7,8].map(s => `<option value="${s}" ${s===sub.semester?'selected':''}>${s}</option>`).join('')}
                            </select>
                        </div>
                    </div>
                    <div id="es-error" class="error-message" style="display:none"></div>
                </div>
                <div class="modal-footer">
        ...
        const closeFn = () => { area.innerHTML = ''; };
        document.getElementById('es-close').addEventListener('click', closeFn);
        document.getElementById('es-overlay').addEventListener('click', closeFn);
        document.getElementById('es-cancel').addEventListener('click', closeFn);
        ...
        document.getElementById('es-save').addEventListener('click', () => {
            ...
            const semester = parseInt(document.getElementById('es-semester').value);
            ...
            DataStore.updateSubject(sub.id, { name, code, credits, departmentId: deptId, semester });
            closeFn();
